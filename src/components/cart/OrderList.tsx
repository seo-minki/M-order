...
const OrderList = () => {
  ...
  useEffect(() => {
    // 총 갯수
    const sum = newOrderList.reduce((arr, cur) => arr + cur.price * cur.quantity, 0);
    setTotalPrice(sum);

    // 상품 이름
    const sumOrderListName = newOrderList.map((item) => item.productName, []).join(',');
    ...
    <section className="max-w-5xl mx-auto py-[72px]">
      {newOrderList.length > 0 ? (
          ...
          <ul>
            {newOrderList.map((order) => (
              <li key={order.id} className="py-4 relative">
                ...
                >
                  <Image src={CloseIcon} alt="로고" width={16} height={16} priority={true} className="w-4" />
                </ButtonComponent>
                ...
                <div className="flex items-center justify-between max-sm:block">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-blue text-right max-sm:text-center">{order.productName}</h3>
                    ...
                    <div className="flex items-center justify-center mt-4">
                      ...
                      />
                      <span className="mx-2 min-w-[50px] text-center">{order.quantity}</span>
                      <ButtonComponent
                        classNames="bg-blue rounded-md w-9 h-9 text-white"
                        buttonText="+"
        ...
        <div className="min-h-8 flex items-center justify-center flex-col">
          <Image src={ShoppingCardAddIcont} width={150} height={150} alt="쇼핑카트"></Image>
